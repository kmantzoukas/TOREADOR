...
// Translator
import org.cumulus.certificate.model.TAssignment;
import org.cumulus.certificate.model.TExprBooleanAbs;
import org.cumulus.certificate.model.TExprNumericalAbs;
import org.cumulus.certificate.model.TParDecl;
import org.cumulus.certificate.model.TTimeUnit;
import org.cumulus.certificate.model.TTrDecl;
import org.cumulus.certificate.model.TVarDecl;
...
//            throw new java.lang.RuntimeException("Asserts must be enabled!!! Use -enableassertions");
        ...
        for (String keyword : keywordsINT)
            idTypeMap.put(keyword, new IDInfo(keyword, ExpressionType.INT));
        // Certificate Lifecycle/SLA Manager Functions
        for (String f : functions0)
            idTypeMap.put(f, new IDInfo(f, ExpressionType.INT));
        ...
        for (String keyword : keywordsBOOL)
            idTypeMap.put(keyword, new IDInfo(keyword, ExpressionType.BOOL));
        ...
        for (String keyword : keywordsCLOCK)
            idTypeMap.put(keyword, new IDInfo(keyword, ExpressionType.CLOCK));
        ...
        for (String keyword : keywordsFLOAT)
            idTypeMap.put(keyword, new IDInfo(keyword, ExpressionType.FLOAT));
	...
	System.out.println(results[0]); // The Prism model
	System.err.println(results[1]); // The Lisp code
    }
	...
	String[] result = new String[2];
        ...
        Translate.toLisp += ";;;; Lisp:\n";

	result[0] = Translate.toPrism;
	result[1] = Translate.toLisp;
	return result;
    }
}
//
...
class Evaluator implements ReflectiveVisitor {
    static public String GLOB_Current_GTName = null;
    ...
    private String realName(String s) {
        return ((IDInfo) Translate.idTypeMap.get(s)).name;
    }
...
+"\n    res #+nil(jnew \"java.lang.String\" res)))\n";
    ...
    public EvalNode visitObjectTStateTransitionModel(org.cumulus.certificate.model.TStateTransitionModel type) {
        ...
        Translate.timeUnit = visit((TExprNumericalAbs)
                                   timeUnitXML.getExprNumericalAbs().getValue());
        EvalNode timeUnitCode = Translate.timeUnit;
        ...
        //    System.err.println(Translate.prologue);
        ...
        assert (! Translate.issueGuard.equals("false"))
        ...
        result2 +=
            ...
            "//****************************************************************\n" +
            "               & ! (  // no issue\n" +
            Translate.issueGuard +
            ")\n               & ! (  // no refuse\n" +
            Translate.refuseGuard +
            ")\n               & ! (  // no revoke\n" +
            Translate.revokeGuard +
            ")\n               & ! (  // no expire\n" +
            Translate.expireGuard +
            ")\n               -> (event_state'= eTreated);\n" +
            "[event_treated]        (event_state = eTreated) -> (event_state' = eNoEvent);\n" +
            "endmodule\n"
            ;
        return new EvalNode(result + Translate.prologue + result2
                            , resultLisp
    ...
    private void updateIdTypeMap(String s, ExpressionType t) {
        assert (! Translate.idTypeMap.containsKey(s))
            : "ID " + s + " already exists";
        //      System.err.println("TYPE: Adding ID " + s + " with type " + t);
        Translate.idTypeMap.put(s, new IDInfo("u_" + s, t));
    }
    ...
    public EvalNode visitTVarDeclEnumerationVar(org.cumulus.certificate.model.EnumerationVar type) {
        ...
        Translate.prologue += constprologue;
    ...
    public EvalNode visitTTrDeclIssue(org.cumulus.certificate.model.Issue type) {
        ...
        Translate.issueGuard += " | ((" + Translate.issueGuardEachPrism + ") & (" + guardEval.toPrism + "))";
        Translate.issueGuardLisp += "\n"
            + Translate.issueGuardEachLisp
            + "\n     "
            + guardEval.toLisp
            + ")";
        return visitTTRDeclHelper(transitionLabel.toLowerCase()
                                  , "((" + Translate.issueGuardEachPrism + ") & (" + guardEval.toPrism + "))"
                                  , Translate.issueBasicActionEachPrism + " & " + assignsString
                                  , Translate.issueGuardEachLisp
                                  , guardEval.toLisp + ")"
                                  , Translate.issueBasicActionEachLisp
                                  , assignsStringLisp);
    }
    ...
    public EvalNode visitTTrDeclRefuse(org.cumulus.certificate.model.Refuse type) {
        ...
        Translate.refuseGuard += " | ((" + Translate.refuseGuardEachPrism + ") & (" + guardEval.toPrism + "))";
        Translate.refuseGuardLisp += "\n"
            + Translate.refuseGuardEachLisp
            + "\n     "
            + guardEval.toLisp
            + ")";
        return visitTTRDeclHelper(transitionLabel.toLowerCase()
                                  , "((" + Translate.refuseGuardEachPrism + ") & (" + guardEval.toPrism + "))"
                                  , Translate.refuseBasicActionEachPrism + " & " + assignsString
                                  , Translate.refuseGuardEachLisp
                                  , guardEval.toLisp + ")"
                                  , Translate.refuseBasicActionEachLisp
                                  , assignsStringLisp);
    }
    ...
    public EvalNode visitTTrDeclRevoke(org.cumulus.certificate.model.Revoke type) {
        ...
        Translate.revokeGuard += " | ((" + Translate.revokeGuardEachPrism + ") & (" + guardEval.toPrism + "))";
        Translate.revokeGuardLisp += "\n"
            + Translate.revokeGuardEachLisp
            + "\n     "
            + guardEval.toLisp
            + ")";
        return visitTTRDeclHelper(transitionLabel.toLowerCase()
                                  , "((" + Translate.revokeGuardEachPrism + ") & (" + guardEval.toPrism + "))"
                                  , Translate.revokeBasicActionEachPrism + " & " + assignsString
                                  , Translate.revokeGuardEachLisp
                                  , guardEval.toLisp + ")"
                                  , Translate.revokeBasicActionEachLisp
                                  , assignsStringLisp);
    }
    ...
    public EvalNode visitTTrDeclExpire(org.cumulus.certificate.model.Expire type) {
        ...
        Translate.expireGuard += " | ((" + Translate.expireGuardEachPrism + ") & (" + guardEval.toPrism + "))";
        Translate.expireGuardLisp += "\n"
            + Translate.expireGuardEachLisp
            + "\n     "
            + guardEval.toLisp
            + ")";
        return visitTTRDeclHelper(transitionLabel.toLowerCase()
                                  , "((" + Translate.expireGuardEachPrism + ") & (" + guardEval.toPrism + "))"
                                  , Translate.expireBasicActionEachPrism + " & " + assignsString
                                  , Translate.expireGuardEachLisp
                                  , guardEval.toLisp + ")"
                                  , Translate.expireBasicActionEachLisp
                                  , assignsStringLisp);
    }
    ...
    public EvalNode visitTExprNumericalAbsId(org.cumulus.certificate.model.Id type) {
        ...
        IDInfo nameInfo = (IDInfo) (Translate.idTypeMap.get(type.getName()));
